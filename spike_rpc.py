...
# based on the work from these sources
...
class RPC:
  ...
  def send_message(self, name, params = {}, expect_respose = True):
    while True:
  ...
  def getNextMessage(self, timeout = 1):
    start_time = time.time()
    elapsed = 0
    while True:
      pos = self.recv_buf.find(b'\x0d')
...
# Program Methods
  ...
  def start_write_program(self, name, size, slot, created, modified, type = 'python'):
    meta = {'created': created, 'modified': modified, 'name': name, 'type': type, 'project_id': '50uN1ZaRpHj2'}
...
# Light Methods
  ...
  def uploadProgram(self, file, to_slot, type = 'python'):
    with open(file, "rb") as f:
      size = os.path.getsize(file)
      name = file
      now = int(time.time() * 1000)
      start = self.start_write_program(name, size, to_slot, now, now, type)
      bs = start['blocksize']
      id = start['transferid']
      with tqdm(total=size, unit='B', unit_scale=True) as pbar:
        b = f.read(bs)
        while b:
          self.write_package(b, id)
          pbar.update(len(b))
          b = f.read(bs)
